	...
	public static Mat DetectForm(Mat img,MatOfPoint contour) {
		...
		if ((contourArea / (Math.PI*radius[0]*radius[0])) >=0.8 ) {
			...
			if (total == 3  && contourSize>10 ) { // is triangle
				...
				Imgproc.rectangle(img, new Point(rect.x,rect.y), new Point(rect.x+rect.width,rect.y+rect.height), new Scalar (0, 255, 0), 3);
				Mat tmp = img.submat(rect.y,rect.y+rect.height,rect.x,rect.x+rect.width);
				Mat sign = Mat.zeros(tmp.size(),tmp.type());
				tmp.copyTo(sign);
				return null;
...
public static Mat DetectFormim(Mat img,MatOfPoint contour) {
	...
	/*public static double Similitude(Mat object,String signfile) {
		...
		// Conversion du signe de reference en niveaux de gris et normalisation
				...
				Imgproc.cvtColor(panneauref,graySign,Imgproc.COLOR_RGB2GRAY);
		...
		public static double Similitude(Mat src,String objectfile) {
			...
			// Conversion du signe de reference en niveaux de gris et normalisation
					Mat sroadSign= Imgcodecs.imread(objectfile);
					Mat sObject=new Mat();
					Mat object = new Mat();
	...
	public static double SimilitudeDB(Mat src,String objectfile,Connection con) throws Exception {
		...
		DescriptorExtractor orbExtractor=DescriptorExtractor.create(DescriptorExtractor.ORB);
		MatOfKeyPoint signKeypoints=new MatOfKeyPoint();
		//keypoints from database
		KeyPoint[] kyptsign=ConnectDB.resultFindkeypoints(objectfile,con);
		signKeypoints.fromArray(kyptsign);
		Mat objectDescriptor=new Mat(sObject.rows(),sObject.cols(),sObject.type());
		orbExtractor.compute(grayObject, signKeypoints,objectDescriptor);
	    ...
	    List<Point> pts2 = new ArrayList<Point>();
	    for(int i = 0; i<good_matches.size(); i++){
	        pts1.add(signKeypoints.toList().get(good_matches.get(i).queryIdx).pt);
	        pts2.add(signKeypoints.toList().get(good_matches.get(i).trainIdx).pt);
	    }
	    // convertion of data types - there is maybe a more beautiful way
	    Mat outputMask = new Mat();
	    ...
	    MatOfDMatch better_matches_mat = new MatOfDMatch();
	    better_matches_mat.fromList(better_matches);
	    Features2d.drawMatches(sObject, signKeypoints, sroadSign, signKeypoints, better_matches_mat, outputImg);
	  //MaBibliothequeTraitementImage.afficheImage("similitude", outputImg);
	   //System.out.println(better_matches_mat.size(),better_matches_mat.pt.x);
		return better_matches_mat.size().height;
	}
